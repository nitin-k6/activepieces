...
const FolderIcon = ({ isFolderOpen }: { isFolderOpen: boolean }) => {
  return isFolderOpen ? (
    <FolderOpen
      className={cn(
        'border-0 flex-shrink-0 w-4.5 h-4.5 text-gray-700',
        {}
      )}
    />
  ) : (
    <Folder className="flex-shrink-0 w-4 h-4 text-gray-700" />
  );
};
      ...
      >
        <TextWithIcon
          ...
          text={
            <div
              className={cn(
                'flex-grow whitespace-break-spaces break-all text-start truncate text-gray-700 ',
                {}
              )}
            >
              {folder.displayName}
...
const FolderFilterList = ({ refresh }: { refresh: number }) => {
  ...
  const [searchParams, setSearchParams] = useSearchParams(location.search);
  const selectedFolderId = searchParams.get(folderIdParamName);
  const [sortedAlphabeticallyIncreasingly, setSortedAlphabeticallyIncreasingly] =
    useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const form = useForm<CreateFolderFormSchema>({
    resolver: typeboxResolver(CreateFolderFormSchema),
